...
export default function ProdutosPage() {
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      setProducts(data.filter(s => s.itemType === 'PRODUCT'));
    } catch {
      // keep state
    }
    setLoading(false);
    ...
    <div style={{ padding: isMobile ? 16 : 30, maxWidth: 1280, margin: '0 auto' }}>
      ...
      >
        <div>
          ...
          <p style={{ margin: '10px 0 0', color: THEME.textSecondary, fontSize: isMobile ? 13 : 14 }}>
            Gerencie seu catálogo de produtos para venda na loja.
          </p>
        </div>
      ...
      {products.length === 0 ? (
        <div style={{ background: THEME.surface, border: `1px solid ${THEME.border}`, borderRadius: 16, padding: 42, textAlign: 'center' }}>
          ...
          <h3 className="font-display" style={{ margin: 0, color: THEME.textPrimary, fontSize: 26, fontWeight: 600 }}>Nenhum produto cadastrado</h3>
          <p style={{ margin: '10px 0 0', color: THEME.textMuted }}>Crie seu primeiro produto para começar a vender na loja.</p>
        </div>
      ) : (
          ...
          {products.map(product => (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 12 }}>
                <div>
                  <h3 style={{ margin: 0, color: THEME.textPrimary, fontSize: 18, fontWeight: 700 }}>{product.name}</h3>
                  {product.description && <p style={{ margin: '6px 0 0', color: THEME.textSecondary, fontSize: 12 }}>{product.description}</p>}
                  {product.category && (
                    <span style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                      ...
                      padding: '3px 8px',
                      borderRadius: 6,
                      background: product.category.color || THEME.border,
                      color: product.category.color ? '#fff' : THEME.textSecondary,
                      fontSize: 11,
                      fontWeight: 600,
                    }}>
                      {product.category.iconEmoji && <span>{product.category.iconEmoji}</span>}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                  <span
                    style={{
                      background: product.isActive ? '#e9f3eb' : '#f8e9e7',
                      color: product.isActive ? '#5f8f67' : '#b86a5f',
                      padding: '4px 10px',
                      borderRadius: 999,
                      fontSize: 11,
                      fontWeight: 700,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
                <InfoCell label="Preço" value={formatPrice(product.priceCents)} />
                <InfoCell label="Estoque" value={product.stock !== null ? String(product.stock) : 'Ilimitado'} />
              </div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <Field label="Nome do Produto *">
                <Input value={form.name} onChange={(value) => setForm({ ...form, name: value })} placeholder="Ex: Shampoo Premium 300ml" />
              </Field>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <Field label="Badge">
                  <Input value={form.badgeText} onChange={(value) => setForm({ ...form, badgeText: value })} placeholder="Ex: Novo" />
                </Field>
              </div>
              ...
              <Field label="Tipo">
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                  <input
                    ...
                    style={{ width: 18, height: 18, accentColor: THEME.gold }}
                  />
                  <span style={{ fontSize: 14, color: THEME.textPrimary }}>Produto físico (requer entrega/retirada)</span>
                </label>
              </Field>
            </div>
